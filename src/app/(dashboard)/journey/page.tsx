...
export default async function JourneyPage() {
  ...
  completedMissions?.forEach((tm) => {
    const mission = Array.isArray(tm.missions)
      ? (tm.missions[0] as { month: number; phase: number } | null)
      : (tm.missions as unknown as { month: number; phase: number } | null)
    if (mission) {
    ...
    <div className="space-y-8">
      ...
      <div className="bg-white rounded-2xl p-5 shadow-sm border border-gray-100">
        <div className="flex items-center justify-between">
          <div>
            <p className="text-3xl font-black text-teen-purple">
              {xp?.total_xp?.toLocaleString('pt-BR') ?? 0} XP
            </p>
          ...
          <div className="text-right">
            ...
            <p className="text-sm text-gray-500">
              {Object.values(completedByMonth).reduce((a, b) => a + b, 0)} missões aprovadas
            </p>
          </div>
        </div>
      </div>
      ...
      {PHASE_CONFIG.map(({ phase, label, months, color, bg, border, text }) => {
        ...
        return (
          <div
            key={phase}
            className={`rounded-2xl border-2 overflow-hidden transition-all ${isUnlocked ? border : 'border-gray-200'}`}
          >
            {/* Header da fase */}
            <div
              className={`bg-gradient-to-r ${isUnlocked ? color : 'from-gray-300 to-gray-400'} p-4 text-white`}
            >
              <div className="flex items-center justify-between">
                <div>
                  ...
                  return (
                    <div
                      ...
                      className={`bg-white rounded-xl p-3 border transition-all ${
                        isUnlocked ? `${border} hover:shadow-sm` : 'border-gray-100 opacity-50'
                      }`}
                    >
                      <div className="flex items-center justify-between mb-2">
                        <span className="text-lg">{theme.icon}</span>
                        <span
                          className={`text-xs font-black ${isUnlocked ? text : 'text-gray-400'}`}
                        >
                          Mês {month}
                        </span>
                      </div>
      ...
      <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100 text-center">
        ...
        <h3 className="font-black text-gray-800 mb-1">Continue evoluindo!</h3>
        <p className="text-gray-500 text-sm mb-4">
          Cada missão completa te aproxima do seu legado.
        </p>
        <Link
          href="/missions"
